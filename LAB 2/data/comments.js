...
const addComments = async (recipeId,userInfo,comment)=>{
    ...
    fetchRecipe._id = fetchRecipe._id.toString();
    return {new:fetchagain, old:fetchRecipe};
}
...
const LikeUnlike = async(recipeId,userId) =>{
    ...
    if(!fetchRecipe)
    {
        throw {statusCode:400,error:"Recipe with this ID not found"};
    }
    ...
    for(let k=0;k<fetchRecipe.likes.length;k++)
    {
        fetchRecipe.likes[k]=fetchRecipe.likes[k].toString();
    }

    return {new:fetchlast,old:fetchRecipe};
}
...
const RemoveComment = async (recipeId, commentId,userID) =>{
    ...
    if(!fetchRecipe)
    {
        throw {statusCode:400, error:"Recipe not found"};
    }
    ...
    for(let i=0;i<comments.length;i++)
    {
        ...
        {
            Id_match_flag=1;
            if(comments[i].userThatPostedComment._id.toString()===userID)
            {
                Id_match_flag=2;
                break;
            }
    ...
    //Either No Comment with given ID found or the User can only delete the comments made by him
    if(Id_match_flag===0)
    {
        throw {statusCode:400, error:"No Comment with given ID found"};
    }
    if(Id_match_flag===1)
    {
        throw {statusCode:403, error:"User can only delete the comments made by him"};
    }
